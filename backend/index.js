...
app.post("/store-cart-details", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Cart details stored successfully", savedItems });
  } catch (error) {
...
app.post("/store-cart-items", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Cart items stored successfully", alert: true });
  } catch (err) {
    console.error(err);
...
app.post("/signup", async (req, res) => {
  ...
  if (password !== confirmPassword) {
    return res
      .status(400)
      .json({ message: "Passwords do not match", alert: false });
  }
  ...
  try {
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ message: "User already exists", alert: false });
    }
    ...
    res.status(201).json({ message: "Signup successful", alert: true });
  } catch (err) {
